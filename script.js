...
const PASSWORD = "fuchsbau-2025";
const AUTH_KEY = "fuchsgeier_auth";
let allJobs = [];
let filteredJobs = [];
let searchTerm = "";
...
function applyFilters(){
  ...
  filteredJobs = allJobs.filter(job=>{
    if(priorityFilter!=="all" && String(job.priority||1)!==priorityFilter) return false;
    if(!term) return true;
...
function renderJobs(){
  ...
  if(!filteredJobs.length){
    container.innerHTML = `<div class="empty-state"><h3>Keine Treffer für diese Filter</h3><p>Andere Stichwörter oder eine andere Priorität ausprobieren.</p></div>`;
  ...
  const items = filteredJobs.map(job=>{
    ...
    return `<article class="job-card"><div class="job-top"><div class="job-title">${title}</div><div class="pill ${prioClass}">${prioLabel}</div></div><div class="job-meta"><span><strong>Quelle:</strong> ${source}</span>${fetched?`<span><strong>Gefunden:</strong> ${fetched}</span>`:""}</div><div class="job-actions"><a class="job-link" href="${url}" target="_blank" rel="noopener noreferrer">Zur Anzeige</a></div></article>`;
  });
...
async function loadConfig(){
  ...
  try{
    ...
    keywordsEl.innerHTML = terms.map(t=>`<span class="pill pill-soft">${t}</span>`).join("");
  }catch{
